...
def _generate_ass(
    ...
    background: str = "semi_transparent_dark",
):
    ...
    """
    # ASS header
    margin_v = int(video_height * 0.25) if position == "lower_third" else 0
    alignment = 5 if position == "center" else 2
    bold = -1 if str(font_weight).lower() == "bold" else 0
    border_style = 1 if background == "none" else 3
    primary = _hex_to_ass(text_color, include_alpha=True, fallback="&H00FFFFFF")
...
Style: Default,{font_family},{font_size},{primary},&H000000FF,&H00000000,&H80000000,{bold},0,0,0,100,100,0,0,{border_style},3,0,{alignment},40,40,{margin_v},1
...
"""
    ...
    ass_highlight = _hex_to_ass(highlight_color, fallback="&H00FFFF&")
...
def _hex_to_ass(color: str, include_alpha: bool = False, fallback: str = "&H00FFFF&") -> str:
    value = str(color).lstrip("#")
    if len(value) != 6 or any(ch not in "0123456789abcdefABCDEF" for ch in value):
        return fallback
    prefix = "00" if include_alpha else ""
    return f"&H{prefix}{value[4:6]}{value[2:4]}{value[0:2]}&"
