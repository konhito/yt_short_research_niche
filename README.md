...
- b roll and thumbnail image generation through OpenAI Images, with Gemini fallback frames
...
python -m verticals serve --port 5000
```

### Backend server
```bash
python -m verticals serve --host 127.0.0.1 --port 5000
```

The backend exposes JSON endpoints for job submission and polling:
...
- `GET /api/jobs/<job_id>`
- `POST /api/jobs/draft`
- `POST /api/jobs/produce`
- `POST /api/jobs/run`

It reads secrets from environment variables first, then from `.env` in the repo root, then from `~/.verticals/.env` if present.
```
...
| Provider | Cost | Setup | Notes |
|----------|------|-------|-------|
| **OpenAI Images** | Paid API | `OPENAI_API_KEY` | Default image provider. |
| **Gemini Imagen** | Free tier available | `GEMINI_API_KEY` | Legacy fallback image provider. |
| **Pexels Videos** | Free API | `PEXELS_API_KEY` | Stock video clips for reaction/action beats. |
| **Pixabay Videos** | Free API | `PIXABAY_API_KEY` | Secondary stock video fallback when Pexels misses. |
...
This creates the script and metadata with a local LLM. Full video production still needs visuals, TTS, captions, and ffmpeg; Edge TTS is free, while OpenAI visuals require `OPENAI_API_KEY`.
...
| `IMGFLIP_USERNAME` | For meme-heavy niche editing | Free static Imgflip memes |
| `IMGFLIP_PASSWORD` | For meme-heavy niche editing | Free static Imgflip memes |
...
You can add reusable clips under `assets/<niche>/...` or `~/.verticals/assets/<niche>/...`.
For example:

```text
assets/gaming/angry/angry_gamer_reaction.mp4
assets/gaming/controller/rgb_controller_closeup.mp4
assets/common/crowd/crowd_cheering.mp4
```

The video resolver checks local clips before stock APIs, then falls back to Pexels, Pixabay, and finally AI images.
...
```text
work_<job>_<lang>/harvested_shorts/harvest_manifest.json
work_<job>_<lang>/harvested_reddit/reddit_harvest_manifest.json
```

Each candidate records its source URL, title, relevance score, duration, dimensions, orientation, SHA-256 media hash, status, rejection reason, and contact-sheet path. Duplicate cross-posts are collapsed before editor planning.
