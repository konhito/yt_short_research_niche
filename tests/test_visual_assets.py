...
def test_resolves_mixed_assets_in_plan_order(monkeypatch, tmp_path):
    ...
    assert [item["source"] for item in assets] == ["imgflip", "pexels"]
...
def test_resolve_visual_assets_never_calls_openai_for_failed_provider(monkeypatch, tmp_path):
    monkeypatch.setattr("verticals.visual_plan.fetch_local_footage", lambda *args, **kwargs: [])
    monkeypatch.setattr("verticals.visual_plan.fetch_pexels_footage", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("down")))
    monkeypatch.setattr("verticals.visual_plan.fetch_pixabay_footage", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("down")))

    assets = resolve_visual_assets([{"type": "pexels", "query": "Anthropic Claude"}], tmp_path)
